...
static void basename_no_ext(const char* path, char* out, size_t outsz) {
  ...
  if (!name || (name2 && name2 > name)) name = name2;
#endif
...
int main(int argc, char** argv) {
  if (argc < 3) {
    fprintf(stderr, "Usage: %s output.zip image1.png [image2.png ...]\n", argv[0]);
    return 1;
  }

  int zerr = 0;

  zip_t* zout = zip_open(argv[1], ZIP_CREATE | ZIP_TRUNCATE, &zerr);
  ...
  for (int i = 2; i < argc; i++) {
    ...
    snprintf(entry, sizeof(entry), "%s.gtex", name);
    ...
    if (zip_file_add(zout, entry, src, ZIP_FL_OVERWRITE) < 0) {
  ...
  if (zip_close(zout) != 0) {
    fprintf(stderr, "Failed to finalize zip\n");
    return 1;
  }
